...
mod test {
    ...
    /// Regression for #9420: a URL resolves to the key the store actually holds.
    ///
    /// `~` is unreserved per RFC 3986 §2.3 and survives the URL verbatim; characters a URL has to
    /// escape decode back to their literal form. Percent-encoding the key here would be re-encoded
    /// by the request layer (`%7E` -> `%257E`), addressing a key no object has.
    #[rstest]
    ...
    /// A path resolved against an explicitly-passed store is already the literal key, so it must
    /// pass through untouched. `Path::from` used to percent-encode it here.
    #[rstest]
