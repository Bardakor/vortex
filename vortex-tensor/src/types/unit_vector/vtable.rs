...
impl ExtVTable for UnitVector {
    ...
    fn can_coerce_to(source: &ExtDType<Self>, target: &DType) -> bool {
        let DType::Extension(target) = target else {
            return false;
        };
        if !target.is::<Vector>() {
            return false;
    ...
    fn least_supertype(source: &ExtDType<Self>, other: &DType) -> Option<DType> {
        let DType::Extension(other) = other else {
            return None;
        };
        let other_is_unit = other.is::<Self>();
        if !other_is_unit && !other.is::<Vector>() {
            return None;
        }

        let storage = source
            .storage_dtype()
            .least_supertype(other.storage_dtype())?;
        ...
        {
            let unit = ExtDType::<Self>::try_new(EmptyMetadata, storage).ok()?;
            return Some(DType::Extension(unit.erased()));
        }

        let vector = ExtDType::<Vector>::try_new(EmptyMetadata, storage).ok()?;
        Some(DType::Extension(vector.erased()))
    }
