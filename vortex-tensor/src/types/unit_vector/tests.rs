...
fn vector_dtype(ptype: PType, dimensions: u32) -> VortexResult<DType> {
    let dtype = ExtDType::<Vector>::try_new(EmptyMetadata, storage_dtype(ptype, dimensions))?;
...
#[test]
fn unit_vector_coerces_and_casts_to_vector() -> VortexResult<()> {
    let array = unit_vector(2, &[0.6, 0.8])?;
    let target = vector_dtype(PType::F64, 2)?;

    assert!(target.can_coerce_from(array.dtype()));
    assert!(!array.dtype().can_coerce_from(&target));

    let cast = array.cast(target.clone())?;
    assert_eq!(cast.dtype(), &target);
    Ok(())
}

#[test]
fn mixed_least_supertype_is_vector_and_symmetric() -> VortexResult<()> {
    let unit = unit_dtype(PType::F32, 4)?;
    let vector = vector_dtype(PType::F64, 4)?;
    let expected = vector.clone();

    assert_eq!(unit.least_supertype(&vector), Some(expected.clone()));
    assert_eq!(vector.least_supertype(&unit), Some(expected));
    Ok(())
}

#[test]
fn precision_widening_erases_unit_refinement() -> VortexResult<()> {
    let f32_unit = unit_dtype(PType::F32, 4)?;
    let f64_unit = unit_dtype(PType::F64, 4)?;
    let expected = vector_dtype(PType::F64, 4)?;

    assert_eq!(f32_unit.least_supertype(&f64_unit), Some(expected.clone()));
    assert_eq!(f64_unit.least_supertype(&f32_unit), Some(expected));
    Ok(())
}
