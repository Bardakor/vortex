...
impl ScalarFnVTable for CosineSimilarity {
    ...
    fn execute(
        ...
        match (lhs_is_unit, rhs_is_unit) {
            (true, true) => {
                return InnerProduct::try_new_array(lhs_ref, rhs_ref)?
                    .into_array()
                    .execute(ctx);
            }
            (true, false) => {
...
impl CosineSimilarity {
    fn execute_one_unit(
        ...
        let dot: PrimitiveArray = InnerProduct::try_new_array(unit, plain.clone())?
            .into_array()
            .execute(ctx)?;
        let plain_norm: PrimitiveArray = L2Norm::try_new_array(plain)?.into_array().execute(ctx)?;
...
mod tests {
    ...
    fn both_unit_vectors_omit_norms() -> VortexResult<()> {
        ...
        let result = CosineSimilarity::try_new_array(lhs, rhs)?
            .into_array()
    ...
    fn one_unit_vector_omits_its_norm() -> VortexResult<()> {
        ...
        let result = CosineSimilarity::try_new_array(unit, plain)?
            .into_array()
            .execute::<PrimitiveArray>(&mut SESSION.create_execution_ctx())?;
