...
impl FileSystem for ObjectStoreFileSystem {
    ...
    async fn delete(&self, path: &str) -> VortexResult<()> {
        // Literal, like every other method here. `from_url_path` would percent-*decode* the key,
        // so a real object named `a%20b.vortex` would resolve to `a b.vortex` — a different
        // object, and one this method would then happily delete.
