    ...
    Fail: Copy + Default + BitOrAssign,
{
...

    {
        ...
        if let Some(views) = Args::per_row_views(&columns) {
            ...
            // check and prevents vectorization of mixed constant and per-row arithmetic. Recheck
            // the optimized IR and mixed-constant benchmarks before restoring that range loop.
