...
//! positions and visit only the set bits of a mixed validity mask, falling back when either the
//! input representation or sink lacks that capability.
...
mod tests {
    ...
    fn test_skip_invalid_sink_rechecks_rows_after_initialization() -> VortexResult<()> {
        ...
        assert!(
            error
                .to_string()
                .contains("after initializing skipped rows"),
