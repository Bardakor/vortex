...
pub(super) const SUM_FIELD: &str = "sum";
pub(super) const IS_OVERFLOW_FIELD: &str = "is_overflow";
pub(super) const IS_EMPTY_FIELD: &str = "is_empty";
...
impl AggregateFnVTable for Sum {
    ...
    fn grouped_state(
        ...
        let return_dtype = self
            .return_dtype(options, input_dtype)
            .ok_or_else(|| vortex_err!("Unsupported sum dtype: {input_dtype}"))?;
        Ok(Box::new(SumGroupedState::try_new(
            partial_dtype.clone(),
            return_dtype,
        )?))
    }
...
pub(super) fn decode_sum_partial_scalar(scalar: Scalar) -> VortexResult<(Scalar, bool, bool)> {
...
mod arithmetic_tests {
    ...
    fn accumulate_list(
        ...
        let grouped: GroupedArray =
            if let Some(groups) = groups.as_opt::<crate::arrays::FixedSizeList>() {
                groups.into_owned().into()
            } else if let Some(groups) = groups.as_opt::<crate::arrays::ListView>() {
                groups.into_owned().into()
            } else {
    ...
    fn run_grouped_sum(groups: &ArrayRef, elem_dtype: &DType) -> VortexResult<ArrayRef> {
        ...
        let mut ctx = array_session().create_execution_ctx();
        let num_groups = accumulate_list(&mut acc, groups, &mut ctx)?;
    ...
    fn grouped_sum_finish_resets() -> VortexResult<()> {
        ...
        let num_groups = accumulate_list(&mut acc, &groups1.into_array(), &mut ctx)?;
        let result1 = acc.finish(num_groups)?;
        ...
        let num_groups = accumulate_list(&mut acc, &groups2.into_array(), &mut ctx)?;
        let result2 = acc.finish(num_groups)?;
